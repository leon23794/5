...
lsb_option = "\x44\x65\x65\x70\x46\x61\x6b\x65"


def enhanced(img: Image, filename: str):
    l.hide(convert_to_sd(img), lsb_option).save(filename)


def check(img: Image) -> Union[str, None]:
    try:
        return l.reveal(img)
    except Exception:
        return None
